  ...
  return Promise.resolve()
    ...
    .then(function() {
      return knex.schema.createTable('entry', function(tbl) {
        ...
        tbl.text('authorIdFk').notNullable().references('authorId').inTable('author').onDelete('CASCADE');
        tbl.text('bookIdFk').notNullable().references('bookId').inTable('book').onDelete('CASCADE');
      });
    });
};
